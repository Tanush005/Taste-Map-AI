...
#             font-style: italic;
#             color: #94a1b2;
#             overflow-wrap: break-word; /* FIX: Wraps long text */
#             word-break: break-word;   /* FIX: Breaks words to prevent overflow */
#         }
#     </style>
# """, unsafe_allow_html=True)
#
...
#     status_class = "card-status-open" if open_status == "Open" else "card-status-closed"
#
#     # --- FIX: Build HTML components separately to avoid f-string issues ---
#     website_link = f'<a href="{result["website"]}" target="_blank">🌐 Website</a>' if result.get("website") else ''
#     phone_info = f'<span>📞 {result["phone"]}</span>' if result.get("phone") else ''
#     review_html = f'<div class="card-review">{result["review"]}</div>' if result.get("review") else ''
#
#     st.markdown(f"""
#         <div class="result-card">
#             <div class="card-image">
...
def display_result_card(result):
    ...
    # --- FIX: Build HTML components separately to prevent f-string rendering errors ---
    website_link = f'<a href="{result.get("website", "")}" target="_blank">🌐 Website</a>' if result.get("website") else ''
    phone_info = f'<span>📞 {result.get("phone", "")}</span>' if result.get("phone") else ''
    review_html = f'<div class="card-review">{result.get("review", "")}</div>' if result.get("review") else ''
